...
func init() {
	...
	AvroSchemaCache.Add("manifest_list_file_v2", Must(avro.NewRecordSchema("manifest_file", "", []*avro.Field{
		...
		Must(avro.NewField("added_files_count",
			...
			WithFieldID(504))),
		Must(avro.NewField("added_data_files_count",
			IntSchema,
			avro.WithDoc("Added entry count (Athena name for field 504)"),
			avro.WithDefault(int32(0)),
			WithFieldID(504))),
		Must(avro.NewField("existing_files_count",
			IntSchema,
			...
			avro.WithDefault(int32(0)),
			WithFieldID(505))),
		Must(avro.NewField("existing_data_files_count",
			IntSchema,
			avro.WithDoc("Existing entry count (Athena name for field 505)"),
			avro.WithDefault(int32(0)),
			WithFieldID(505))),
		Must(avro.NewField("deleted_files_count",
			IntSchema,
			...
			avro.WithDefault(int32(0)),
			WithFieldID(506))),
		Must(avro.NewField("deleted_data_files_count",
			IntSchema,
			avro.WithDoc("Deleted entry count (Athena name for field 506)"),
			avro.WithDefault(int32(0)),
			WithFieldID(506))),
		Must(avro.NewField("partitions",
			NullableSchema(
